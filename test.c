            ...
            lzfxs_fn decompressor)
{
  ...
  real_length        = ((int)( ilen * 1.05 ) == ilen ? ilen + 20 : (int)ilen * 1.05 )
                       + GUARD_BYTES;
  ...
  if (memcmp(
        comparison_buffer,
        compressed_buffer + size_after_compression,
        GUARD_BYTES))
    {
...
int
perform_tests(const void *ibuf, unsigned int ilen, const char *fname)
{
  int  nfailed  = 0;
  int  rc       = 0;
